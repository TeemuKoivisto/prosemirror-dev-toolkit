...
describe('createHistoryEntry', () => {
  beforeAll(() => {
    ...
    view = setupEditor(el)
    vi.useFakeTimers()
    vi.setSystemTime(new Date(2024, 0, 1, 2))
    const oldMath = Math
    vi.stubGlobal('Math', {
      floor: oldMath.floor,
      max: oldMath.max,
