...
export function mapDocDeltaChildren(delta: Delta, type: ValueType) {
  ...
  if (type === 'array' && d[1] === 0 && d[2] === 0) {
    // Remove operation is defined by two 0s in its delta [{ <deleted> }, 0, 0]
    return []
    // } else if (type === 'array' && typeof d[0] === 'string' && typeof d[1] === 'number' && d[2] === 3) {
    ...
    // with them for now.
  } else if (type === 'array' && typeof d[0] === 'string' && d[1] === 0 && d[2] === 2) {
    ...
    // Eg ["@@ -1,4 +1,9 @@\n-text\n+paragraph\n", 0, 2]
    return []
  } else if (type === 'array' && d.length === 1 && typeof d[0] === 'object') {
  ...
  for (const key in d) {
    ...
    if (key.charAt(0) === '_') {
      transformed.push([key.substr(1), d[key]])
    } else {
      transformed.push([key, d[key]])
    }
  }
  return transformed
}
